...
from sysManage.positionEngineer.AddUnitChoose import AddPosenginUnitChoose
...
class DangerGoodDirectoryMaintenance(QWidget, Widget_Dict_Set):
    ...
    def _initUnitTreeWidget(self, root, mother):
        if root == '':
            result = selectPosenginUnitInfoByDeptUper('')
        else:
            result = selectPosenginUnitInfoByDeptUper(root)
    ...
    def _initUnitTableWidget(self):
        result = selectAllDataAboutPosenginUnit()
    ...
    def _initEquipTableWidget(self):
        result = selectAllPoseginDirectory()
        ...
        # 装备目录
            ...
            if self.le_equipID.text() == "" or self.le_equipName.text() == "":
                ...
                if haveUperID == False:
                    ...
                    insertOneDateIntoEquipmentDirectory(Equip_ID, Equip_Name, Equip_Uper, Input_Type, Equip_Type)
                self.slotEquipDictInit()

    def updateUnit(self):
        insertIntoPosenginUnitFromList(self.addUnitChoose.currentUnitID)
        ...
        if self.changeUnit == '1':
            if (self.tb_result.item(self.tb_result.currentRow(),
                ...
                if haveUperID == False:
                    ...
                    insertOneDateIntoUnitDirectory(Unit_ID, Unit_Name, Unit_Uper)
                    self.slotUnitDictInit()
            pass
