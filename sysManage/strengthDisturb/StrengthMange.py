...
from PyQt5.QtWidgets import QWidget, QTreeWidgetItem, QTableWidgetItem, QAbstractItemView, \
    QMessageBox, QListWidgetItem, QInputDialog, QHeaderView, QLineEdit, QFileDialog
...
from widgets.strengthDisturb.strengthMaintenMange import StrengthMaintenMangeUI

regx = QRegExp("[0-9]*")
'''
   实力维护
'''
class StrengthManage(QWidget, StrengthMaintenMangeUI):
    def __init__(self, parent=None):
        super(StrengthManage, self).__init__(parent)
        self.setupUi(self)
    ...
    def signalConnect(self):
        ...
        #从Excel导入
        self.pb_inputFromExcel.clicked.connect(self.slotInputFromExcel)
        #导出数据包
        ...
        if self.currentColumn == 2:
            for i, resultRowInfo in self.currentInquiryResult.items():
                if i == self.currentRow:
                    ...
                    equipHaveChild = selectEquipIsHaveChild(resultRowInfo[2])
                    if isHavePulicEquip(resultRowInfo[2]):
                        getMessageBox('录入', '该单位或装备不是末级，无法修改', True, False)
                        self.tw_result.cellWidget(self.currentRow, 2).setText(str(resultRowInfo[4]))
                        return
                    if unitHaveChild or equipHaveChild:
                        ...
                        try:
                            if self.tw_result.item(self.currentRow, self.currentColumn).text() != resultRowInfo[2]:
                                newStrength = self.tw_result.item(self.currentRow, self.currentColumn).text()
                                newStrength = int(newStrength)
                                updateSuccess = updateStrengthAboutStrengrh \
                                    (resultRowInfo[0], resultRowInfo[2], self.year, newStrength, str(resultRowInfo[4]))
                            ...
                            getMessageBox('修改', '只能修改为整数?', True, False)
                            self.tw_result.item(self.currentRow, 2).setText(str(resultRowInfo[4]))
                            self._initTableWidgetByUnitListAndEquipList(self.unitList, self.equipList, self.year)
                            return
            ...
            for i, resultRowInfo in self.currentInquiryResult.items():
                if i == self.currentRow:
                    ...
                    if unitHaveChild or equipHaveChild:
                        ...
                        try:
                            if self.tw_result.item(self.currentRow, self.currentColumn).text() != resultRowInfo[5]:
                                newPlanUnhanded = self.tw_result.item(self.currentRow, self.currentColumn).text()
                                newPlanUnhanded = int(newPlanUnhanded)
                                updateSuccess = updatePlanUnhanded(resultRowInfo[0], resultRowInfo[2], self.year, newPlanUnhanded, str(resultRowInfo[5]))
                                if not updateSuccess:
                                    getMessageBox("修改", str(updateSuccess) + "修改失败", True, False)
                                    # QMessageBox.information(self, "修改", "修改成功！", QMessageBox.Yes)
                                self._initTableWidgetByUnitListAndEquipList(self.unitList, self.equipList, self.year)
                            ...
                            getMessageBox('修改', '只能修改为整数?', True, False)
                            self.tw_result.item(self.currentRow, 3).setText(str(resultRowInfo[5]))
                            self._initTableWidgetByUnitListAndEquipList(self.unitList, self.equipList, self.year)
                            return
        else:
            return
    ...
    def slotClearAllRow(self):
        ...
        for i, resultInfo in self.currentInquiryResult.items():
            ...
            Equip_ID = resultInfo[2]
            orginStrength = resultInfo[4]
            orginPlanUnhandled = resultInfo[5]
            year = resultInfo[-1]
            ...
            if unitHaveChild or equipHaveChild:
                ...
                updateStrengthSuccess = updateStrengthAboutStrengrh \
                    (Unit_ID, Equip_ID, self.year, 0, orginStrength)
                updatePlanUnhandedSuccess = updatePlanUnhanded(Unit_ID, Equip_ID, self.year,
                                                               0, orginPlanUnhandled)
                if updateStrengthSuccess != True or updatePlanUnhandedSuccess != True:
                    getMessageBox('清除', '第' + str(i + 1) + "行清除失败。" , True, False)
                    return
                updateOneEquipmentBalanceData(year, Equip_ID, Unit_ID)
                # getMessageBox('成功', '第' + str(i + 1) + "行清除成功", True, False)
                self._initTableWidgetByUnitListAndEquipList(self.unitList, self.equipList, self.year)
    ...
    def slotClearCurrentRow(self):
        ...
        for i, resultRowInfo in self.currentInquiryResult.items():
            if i == currentRow:
                ...
                equipHaveChild = selectEquipIsHaveChild(resultRowInfo[2])
                if isHavePulicEquip(resultRowInfo[2]):
                    getMessageBox('清除', '该单位或装备不是末级，无法清除', True, False)
                    self.tw_result.cellWidget(self.currentRow, 2).setText(str(resultRowInfo[4]))
                    return
                if unitHaveChild or equipHaveChild:
                    getMessageBox('清除', '该单位或装备不是末级，无法清除', True, False)
                    ...
                    updateStrengthSuccess = updateStrengthAboutStrengrh \
                        (resultRowInfo[0], resultRowInfo[2], self.year, 0, str(resultRowInfo[4]))
                    updatePlanUnhandedSuccess = updatePlanUnhanded(resultRowInfo[0], resultRowInfo[2], self.year,
                                                                   0, str(resultRowInfo[5]))
                    if not updateStrengthSuccess:
                        getMessageBox("清除", str(updateStrengthSuccess) + "清除失败", True, False)
                    if not updatePlanUnhandedSuccess:
                        getMessageBox("清除", str(updatePlanUnhandedSuccess) + "清除失败", True, False)
                    self._initTableWidgetByUnitListAndEquipList(self.unitList, self.equipList, self.year)
    ...
    def initEquipTreeWidget(self, stack, root):
        while stack:
            ...
            item = QTreeWidgetItem(root.pop(0))
            #设置级联选中
            item.setFlags(Qt.ItemIsUserCheckable | Qt.ItemIsEnabled | Qt.ItemIsSelectable | Qt.ItemIsEditable | Qt.ItemIsAutoTristate)
            item.setText(0, EquipInfo[1])
    ...
    def slotInquryStrengthResult(self):
        ...
        # self.updateParentItemStatus(QTreeWidgetItem(self.tw_first))
        # self.updateChildItemStatus(QTreeWidgetItem(self.tw_first))
        # self.updateParentItemStatus(QTreeWidgetItem(self.tw_first))
        # self.updateChildItemStatus(QTreeWidgetItem(self.tw_second))
        # self.updateParentItemStatus(QTreeWidgetItem(self.tw_second))
        ...
        #     self.tw_result.setRowCount(0)
        #     return
        if len(self.currentCheckedUnitList) > 0 and len(self.currentCheckedEquipList) > 0:
    ...
    def _initTableWidgetByUnitListAndEquipList(self, UnitList, EquipList, year):
        ...
        if self.rb_equipShow.isChecked():
            # 按装备展开
            self.resultList = selectAboutStrengthMaintanceByEquipShow(UnitList, EquipList, year)
        elif self.rb_unitShow.isChecked():
            # 按单位展开
            self.resultList = selectAboutStrengthMaintanceByUnitShow(UnitList, EquipList, year)
        else:
            self.resultList = selectAboutStrengthMaintanceByUnitListAndEquipList(UnitList, EquipList, year)

        if self.cb_showLast.isChecked():
            self.resultList = selectAboutStrengthMaintanceByLast(UnitList, EquipList, year)
        ...
        headerlist = ['单位名称', '装备名称', '计划数', '计划未移交量']
        self.tw_result.setHorizontalHeaderLabels(headerlist)
        self.currentInquiryResult.clear()
        self.tw_result.setColumnCount(len(headerlist))
        ...
        for LineInfo in self.resultList:
            #[UnitID, UnitName, EquipID, EquipName, strengthNum, planUnhandedNum, year]
            item = QTableWidgetItem(str(LineInfo[1]))
    ...
    def slotOutputToExcel(self):
        ...
        if len(directoryPath) > 0:
            ...
            #画表头
            headerlist = ['单位编号','单位名称', '装备编号', '装备名称', '计划数', '计划未移交量', '年份']
            ...
            for index,LineInfo in enumerate(self.resultList):
                ...
                workSheet.write(index + 1, 0, str(LineInfo[0]), contentStyle)
                workSheet.write(index + 1, 1, str(LineInfo[1]), contentStyle)
                workSheet.write(index + 1, 2, str(LineInfo[2]), contentStyle)
                workSheet.write(index + 1, 3, str(LineInfo[3]), contentStyle)
            ...
            try:
                pathName = "%s/%s年%s计划数与计划未移交量表.xls" % (directoryPath,self.year, selectUnitNameByUnitID(self.unitList[0]))
                ...
                getMessageBox("导出失败", "检查是否Excel是否存在或Excel是否被占用！", True, False)
                return
        else:
            getMessageBox("未选中导出目录", "导出！", True, False)
            return
        pass
    def slotInputFromExcel(self):
        self.inputList = []
        ...
        try:
            ...
            if self.workSheet.ncols != 7:
                getMessageBox("导入失败", "打开文件失败，请检查文件是否正确", True, False)
                self.setDisabled(False)
                return
            title = ['单位编号','单位名称', '装备编号', '装备名称', '计划数', '计划未移交量', '年份']
            for i in range(len(title)):
                if self.workSheet.cell(0, i).value != title[i]:
                    getMessageBox("导入失败", "导入失败，请检查文件是否匹配！", True, False)
                    self.setDisabled(False)
                    return
            ...
            for r in range(1, self.workSheet.nrows):
                ...
                planUnhanded = (self.workSheet.cell(r, 5).value)
                year = (self.workSheet.cell(r, 6).value)
                try:
                    unitId = str(int(unitId))
                    equipId = str(int(equipId))
                    strength = str(int(strength))
                    planUnhanded = str(int(planUnhanded))
                except:
                ...
                inputInfo.append(planUnhanded)
                inputInfo.append(year)
                self.inputList.append(inputInfo)
            self.showInputResult.setWindowTitle("从Excel导入%s年计划数与计划未移交量" % year)
    ...
    def slotOutputData(self):
        ...
        if len(directoryPath) > 0:
            ...
            if dataList is None or len(dataList) == 1:
                ...
                pathName = "%s/%s计划数与计划未移交量.nms" % (directoryPath, installData)
    ...
    def slotInputData(self):
        ...
        try:
            with open(filename, "rb") as file:
                inputList = pickle.load(file)
                if inputList[0] != "计划数与计划未移交量":
        ...
        self.showInputResult.tw_result.setRowCount(len(inputList) - 1)
        print("self.inputList", self.inputList)
        for i, LineInfo in enumerate(inputList):
            if i == 0:
                continue
            i = i - 1
            item = QTableWidgetItem(str(LineInfo[0]))
            item.setFlags(Qt.ItemIsSelectable | Qt.ItemIsEnabled)
            ...
            self.inputList.append(LineInfo)
        self.showInputResult.show()
    ...
    def slotInputIntoDatabase(self):
        ...
        for i, lineInfo in enumerate(self.inputList):
            try:
                ...
                orginPlanUnhanded = selectPlanUnhandedNum(lineInfo[0], lineInfo[2], lineInfo[-1])
                    ...
                    if unitHaveChild or equipHaveChild:
                        ...
                        if orginStrengthNum != int(lineInfo[4]):
                            if not updateStrengthAboutStrengrh(lineInfo[0], lineInfo[2], lineInfo[-1], int(lineInfo[4]),
                                                               orginStrengthNum):
                                raise Exception
                        elif orginPlanUnhanded != int(lineInfo[5]):
                            if not updatePlanUnhanded(lineInfo[0], lineInfo[2], lineInfo[-1], int(lineInfo[5]),
                                                               orginStrengthNum):
                                raise Exception
