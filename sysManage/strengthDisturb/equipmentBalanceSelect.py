...
class Equip_Balance_Select(QWidget, EquipmentBalanceSelectUI):
        ...
        if resultList is not None and len(resultList) is not 0:
            ...
            for row in range(len(resultList)):
                ...
                self.tb_result.setItem(row + 3,  3,
                                       QTableWidgetItem(resultList[row].get('authorized_value_increase', )))
                self.tb_result.setItem(row + 3, 4,
                                       QTableWidgetItem(resultList[row].get('authorized_value_decrease', )))
                ...
                self.tb_result.setItem(row + 3, 66,
    ...
    def _initTableHeader(self):
        ...
        self.tb_result.setSpan(0, 5, 3, 1)

        item = QTableWidgetItem("质量状况")
        item.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
        self.tb_result.setItem(0, 6, item)
        self.tb_result.setSpan(0, 6, 3, 8)

        item = QTableWidgetItem("下发")
        item.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
        self.tb_result.setItem(1, 6, item)
        self.tb_result.setSpan(1, 6, 3, 4)

        item = QTableWidgetItem("上报")
        item.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
        self.tb_result.setItem(1, 10, item)
        self.tb_result.setSpan(1, 10, 3, 4)
        ...
        item = QTableWidgetItem("待退役")
        item.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
        self.tb_result.setItem(2, 13, item)
        ...
        self.tb_result.setSpan(0, 15, 3, 1)

        item = QTableWidgetItem("变化项目（增）")
        item.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
        self.tb_result.setItem(0, 16, item)
        self.tb_result.setSpan(0, 16, 2, 7)

        item = QTableWidgetItem("变化项目（减）")
        item.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
        self.tb_result.setItem(0, 23, item)
        self.tb_result.setSpan(0, 23, 2, 8)
        ...
        item.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
        self.tb_result.setItem(2, 30, item)
